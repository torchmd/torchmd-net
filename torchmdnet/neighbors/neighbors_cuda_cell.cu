...
__device__ int3 getCell(float3 p, float3 box_size, float cutoff) {
    ...
    int cx = floorf((p.x + float(0.5) * box_size.x) / cutoff);
    int cy = floorf((p.y + float(0.5) * box_size.y) / cutoff);
    int cz = floorf((p.z + float(0.5) * box_size.z) / cutoff);
...
static auto fillCellOffsets(const Tensor& sorted_positions, const Tensor& sorted_indices,
                            const Tensor& batch, float3 box_size, float cutoff) {
    ...
    AT_DISPATCH_FLOATING_TYPES(sorted_positions.scalar_type(), "fillCellOffsets", [&] {
        ...
        fillCellOffsetsD<<<blocks, threads, 0, stream>>>(
            ...
            get_accessor<int32_t, 1>(batch), box_size, cutoff);
    });
    ...
    for (int i = 0; i < 27; i++) {
        ...
        if (firstParticle != -1) { // Continue only if there are particles in this cell
            ...
            for (int j = 0; j < nincell; j++) {
                ...
                if (cur_j < i_atom and j_batch == i_batch) {
...
public:
    static tensor_list forward(AutogradContext* ctx, const Tensor& positions, const Tensor& batch,
        ...
        std::tie(cell_start, cell_end) =
            fillCellOffsets(sorted_positions, hash_values, batch, box_size_, cutoff_);
