...
template <typename scalar_t> __global__ void forward_kernel(
    ...
    const Accessor<int32_t, 1> batch_offsets,
...
public:
  static tensor_list forward(AutogradContext* ctx, const Tensor& positions,
        ...
        // Batch is assumed to be of type int32
	  ...
	  for(int i=0; i<n_batches; i++){
	    ...
	    AT_DISPATCH_FLOATING_TYPES(positions.scalar_type(), "get_neighbor_pairs_forward", [&]() {
	      ...
	      forward_kernel<<<num_blocks, num_threads, 0, stream>>>(
								     ...
								     get_accessor<int32_t, 1>(batch_offsets),
								     i,
								     cutoff_ * cutoff_, get_accessor<int32_t, 1>(i_curr_pair),
...
TORCH_LIBRARY_IMPL(neighbors, AutogradCUDA, m) {
    m.impl("get_neighbor_pairs",
           [](const Tensor& positions, const Tensor& batch, const Scalar& cutoff, const Scalar& max_num_pairs, bool checkErrors) {
