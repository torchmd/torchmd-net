...
// In the host, throw if there are more than 1024 cells in any dimension
    ...
    if (cell_dim.x > 1024 || cell_dim.y > 1024 || cell_dim.z > 1024) {
        // SILENCED FOR ROCM:         throw std::runtime_error("Too many cells in one dimension. Maximum is 1024");
    }
#endif
    return cell_dim;
}
...
static auto sortAtomsByCellIndex(const Tensor& positions, const Tensor& box_size,
    ...
    AT_DISPATCH_FLOATING_TYPES(positions.scalar_type(), "assignHash", [&] {
       ...
       hipLaunchKernelGGL(( assignCellIndex), dim3(blocks), dim3(threads), 0, stream, get_accessor<scalar_t, 2>(positions),
...
static auto fillCellOffsets(const Tensor& sorted_cell_indices, int3 cell_dim) {
    ...
    auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA();
   hipLaunchKernelGGL(( fillCellOffsetsD), dim3(blocks), dim3(threads), 0, stream, get_accessor<int32_t, 1>(sorted_cell_indices),
...
forward_cell(const Tensor& positions, const Tensor& batch, const Tensor& in_box_size,
    ...
    const auto stream = getCurrentHIPStreamMasqueradingAsCUDA(positions.get_device());
    { // Traverse the cell list to find the neighbors
        const HIPStreamGuardMasqueradingAsCUDA guard(stream);
        AT_DISPATCH_FLOATING_TYPES(positions.scalar_type(), "forward", [&] {
            ...
            const int threads = 128;
            const int blocks = (num_atoms + threads - 1) / threads;
           hipLaunchKernelGGL(( traverseCellList), dim3(blocks), dim3(threads), 0, stream, cell_list_accessor, list_accessor,
