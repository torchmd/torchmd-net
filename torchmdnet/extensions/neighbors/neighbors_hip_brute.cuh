...
forward_brute(const Tensor& positions, const Tensor& batch, const Tensor& in_box_vectors,
    ...
    const uint64_t num_blocks = ::max(static_cast<uint64_t>((num_all_pairs + num_threads - 1UL) / num_threads), static_cast<uint64_t>(1UL));
    AT_DISPATCH_FLOATING_TYPES(positions.scalar_type(), "get_neighbor_pairs_forward", [&]() {
       ...
       hipLaunchKernelGGL(( forward_kernel_brute), dim3(num_blocks), dim3(num_threads), 0, stream, 
        ...
        if (loop) {
            ...
            const uint32_t num_blocks_self =
                ::max((num_atoms + num_threads_self - 1U) / num_threads_self, 1U);
           hipLaunchKernelGGL(( add_self_kernel), dim3(num_blocks_self), dim3(num_threads_self), 0, stream, 
