...
def create_numpy_arrays(file_path):
    with open(file_path, 'r') as file:
        ...
        for i in range(num_conformations):
            ...
            box_vectors[i] = [float(x) for x in lattice_match.group(1).split()] if lattice_match else None
...
class WaterBox(InMemoryDataset):
    ...
    def download(self):
        ...
        if r.status_code != 200:
            raise Exception(f"Failed to download file from {self.url}. Status code: {r.status_code}")
    ...
    def process(self):
        dataset_xyz_path = os.path.join(self.raw_dir, 'training-set', 'dataset_1593.xyz')
        energies, forces, positions, atomic_numbers, box_vectors = create_numpy_arrays(dataset_xyz_path)
