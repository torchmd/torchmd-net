...
class Custom(Dataset):
    ...
    def __init__(self, coordglob, embedglob, energyglob=None, forceglob=None):
        ...
        if self.has_energies:
            assert len(self.coordfiles) == len(self.energyfiles), (f'Number of coordinate files {len(self.coordfiles)} '
                                                                   f'does not match number of energy files {len(self.energyfiles)}.')
        if self.has_forces:
            assert len(self.coordfiles) == len(self.forcefiles), (f'Number of coordinate files {len(self.coordfiles)} '
        ...
        for i in range(nfiles):
            ...
            # consistency check
            assert coord_data.shape[1] == embed_data.shape[0], (f'Number of atoms in coordinate file {i} ({coord_data.shape[1]}) '
                                                                f'does not match number of atoms in embed file {i} ({embed_data.shape[0]}).')
            if self.has_energies:
                energy_data = np.load(self.energyfiles[i])
                assert coord_data.shape[0] == energy_data.shape[0], (f'Number of frames in coordinate file {i} ({coord_data.shape[0]}) '
                                                                     f'does not match number of frames in energy file {i} ({energy_data.shape[0]}).')
            if self.has_forces:
                force_data = np.load(self.forcefiles[i])
                assert coord_data.shape == force_data.shape, (f'Data shape of coordinate file {i} {coord_data.shape} '
    ...
    def get(self, idx):
        ...
        features = dict(
            pos=torch.from_numpy(coord_data),
            z=torch.from_numpy(embed_data)
        )
