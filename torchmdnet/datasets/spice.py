...
class SPICE(Dataset):
    ...
    def sample_iter(self):
        ...
        for mol_id, mol in tqdm(h5py.File(self.raw_paths).items(), desc='Molecules'):
            ...
            all_y = pt.tensor(mol['formation_energy'], dtype=pt.float64) * self.HARTREE_TO_EV
            all_dy = pt.tensor(mol['dft_total_gradient'], dtype=pt.float32) * self.HARTREE_TO_EV / self.BORH_TO_ANGSTROM
